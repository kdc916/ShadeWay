...
const VERSION = '1.9.4.30-production';
      ...
      for(const ep of UPSTREAM.overpass){
        try{
          const r=await upstreamJson(ep,{
            method:'POST',
            headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8','User-Agent':'ShadeWay/1.9.4.30'},
...
async function proxySearch(url,env,ctx){
  ...
  const out=await cacheJson(`search/${key}`,300,ctx,async()=>{
    const r=await upstreamJson(`${UPSTREAM.nominatim}?${p}`,{
      headers:{'Accept':'application/json','User-Agent':'ShadeWay/1.9.4.30 (walking navigation research)'}
...
async function deepHealth(env,ctx){
  ...
  await Promise.all([
    run('nominatim',async()=>{await upstreamJson(`${UPSTREAM.nominatim}?format=jsonv2&limit=1&q=Seoul`,{headers:{'User-Agent':'ShadeWay/1.9.4.30'}},env,ctx,{provider:'Nominatim',timeout:5000})}),
