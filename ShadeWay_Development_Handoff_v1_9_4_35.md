...
**프로젝트명:** ShadeWay  
**현재 안정 기준:** `ShadeWay_MVP_v1_9_4_35_Live_Compass_Arrow.html`  
**문서 기준 버전:** `v1.9.4.35 Always-Live GPS + Faster Shadows · Cloud Production · Shade Graph Beta`  
**최종 업데이트:** 2026-08-09  
...
- 프로젝트 목표와 차별점
- 초기 v0.1부터 최신 v1.9.4.32까지 개발 흐름
- 현재 구현된 전체 기능
- 현재 기술 구조
- 그림자 계산 방식
- 건물 높이 추정 방식
...
향후 패치는 **v1.9.4.35을 기준으로 누적 개발**하며, 이전 버전으로 롤백하면서 최신 기능을 잃지 않는다.
...
```text
ShadeWay_MVP_v1_9_4_32_Walk_Arrow_Steps_Fix.html
```
...
```text
ShadeWay_MVP_v1_9_4_32_Walk_Arrow_Steps_Fix.html
ShadeWay_Development_Handoff_v1_9_4_32.md
ShadeWay_v1_9_4_32_Walk_Arrow_Steps_Fix.zip
```
...
# v1.9.4.32 — Walk Camera + Direction Arrow Stability
...
# v1.9.4.32 — Walk Arrow + Steps Fix

실제 모바일 보행 테스트에서 발견된 두 가지 회귀를 수정했다.

## A. 진행 방향 화살표 180° 역방향 보정
- Android/브라우저 GPS `coords.heading`이 실제 이동 방향과 약 180° 반대로 들어오는 경우를 감지한다.
- 실제 GPS 이동 벡터를 최우선 방향 신호로 사용한다.
- 이동 벡터가 아직 없을 때는 현재 경로의 forward bearing을 기준으로 역방향 heading을 보정한다.
- 실제 이동 벡터가 확보되면 사용자가 경로 반대쪽으로 걷는 경우도 실제 이동 방향을 그대로 표시한다.
- Heading Up 지도 회전과 현재 위치 화살표가 동일한 안정화 heading을 공유한다.

## B. 걸음 수 UI 의미 분리
- `estimatedSteps`는 보행 시작 후 실제 GPS 이동거리로 계산한 **누적 걸은 걸음**임을 명확히 했다.
- Turn HUD의 `남은 Xm · 약 N걸음`은 이제 남은 경로거리 / 보폭으로 계산한 **남은 예상 걸음**을 표시한다.
- 보행 상태 카드의 항목명은 `예상 걸음` → `걸은 걸음`으로 변경했다.
- 현재 위치 화살표 옆에 따라다니던 `약 N걸음` 지도 배지를 완전히 제거해 지도/경로 가독성을 확보했다.
- 현재 위치 마커 팝업은 필요할 때만 누적 이동 걸음을 표시한다.

## 회귀 보호
- v1.9.4.31 walk camera dead-zone/gesture hold 유지
- Cloudflare Worker + Static Assets 유지
- Shade Graph Beta 유지
- P0/P1 기능 전체 유지
- route-first architecture 유지
- Settings no-jump / map labels top / mobile SHADE TIME 유지

# v1.9.4.33 — Walk Route Arrow Fix

실기기 보행 테스트에서 두 가지 회귀를 수정했다.

1. `따라가기` 중 지도 경로 중간에 표시되던 파란 회전 화살표(`maneuverLayer`)를 제거했다. 회전 방향/거리/도로명은 상단 Turn HUD에서만 안내한다.
2. 현재 위치 화살표는 경로 위에 있을 때 Android GPS `coords.heading`보다 **현재 경로 14m 전방의 정방향 bearing**을 최우선으로 사용한다. 따라서 실제 기기에서 반복 확인된 180° 반전 heading 영향을 받지 않는다.
3. 경로 이탈 상태에서는 실제 이동 벡터 → GPS heading → 경로 방향 순으로 fallback한다.
4. Heading Up 역시 동일한 route-forward heading을 사용해 지도 회전과 사용자 화살표가 같은 기준을 공유한다.

회귀 보호:
- v1.9.4.32 걸음수 UI 수정 유지
- v1.9.4.31 카메라 튐 방지 유지
- Cloudflare Worker / PWA / Shade Graph 유지
- 상단 Turn HUD / 음성 / 진동 안내 유지
...
실기기 보행 테스트에서 현재 위치 화살표가 경로 방향에는 맞춰지지만, 사용자가 제자리에서 휴대폰을 회전해도 화살표가 즉시 회전하지 않는 문제를 수정했다.

## 핵심 변경
- `deviceorientationabsolute`를 우선 사용해 Android/Chromium의 절대 방향 센서를 실시간 수신한다.
- iOS Safari에서는 `DeviceOrientationEvent.requestPermission()`을 `따라가기` 사용자 제스처에서 요청하고 `webkitCompassHeading`을 사용한다.
- Android absolute `alpha`는 지도에서 사용하는 시계방향 compass heading으로 변환한다.
- 센서 이벤트는 `requestAnimationFrame`으로 합쳐 과도한 DOM 갱신을 막고 angular smoothing을 적용한다.
- GPS fix가 새로 오지 않아도 휴대폰을 돌리면 현재 위치 파란 화살표가 즉시 회전한다.
- 센서 heading이 최근 1.8초 이내면 센서를 최우선으로 사용한다.
- 센서가 없거나 권한이 거부되거나 stale이면 기존 route-forward → motion/GPS fallback을 유지한다.
- `Heading Up`이 ON이면 동일한 실시간 compass heading으로 지도 회전도 갱신한다.
- 보행 종료 시 DeviceOrientation listener와 animation frame을 해제해 배터리/이벤트 누수를 방지한다.
- v1.9.4.33의 경로 중간 turn-arrow marker 제거는 그대로 유지한다.

## 실기기 확인
1. 따라가기 시작 후 제자리에서 휴대폰을 좌/우로 돌릴 때 파란 현재 위치 화살표가 즉시 같이 도는지.
2. 360°→0° 경계를 지나도 화살표가 반대 방향으로 크게 튀지 않는지.
3. Android Chrome/Samsung Internet에서 방향이 북/동/남/서와 대체로 일치하는지.
4. iOS에서는 센서 권한 허용 후 회전하는지.
5. 센서 미지원/거부 상태에서도 몇 m 이동하면 기존 GPS/경로 fallback으로 방향이 나오는지.
6. 따라가기 종료 후 센서 이벤트가 계속 동작하지 않는지.


# v1.9.4.35 — Always-Live GPS · Real-time Speed · Faster Shadows

## 핵심 수정
- 경로 탐색/따라가기와 GPS watch 생명주기를 분리했다. 위치 권한이 허용되면 앱 시작 직후 `watchPosition()`을 유지한다.
- 보행 안내를 종료해도 GPS watch를 끄지 않는다. 따라서 목적지/경로가 없어도 현재 위치 마커가 새 좌표로 계속 이동한다.
- watch 이벤트가 6.5초 이상 오지 않으면 고정밀 `getCurrentPosition()`을 보조 호출하는 foreground watchdog을 추가했다. 화면 복귀/pageshow에서도 stale fix를 즉시 갱신한다.
- 모바일 기본 화면에 `이동 속도 0.0 km/h` 실시간 표시를 추가했다. `coords.speed`가 없으면 연속 GPS fix의 거리/시간으로 계산하며 accuracy 기반 jitter 억제를 적용한다.
- ETA 학습 속도와 현재 실시간 표시 속도를 분리했다. ETA는 보행 범위 샘플만 학습하고, 현재 속도는 더 빠른 EMA로 반응한다.
- 현재 위치가 약 8m 이상 이동하면 경로가 없는 상태의 그림자를 빠르게 재렌더한다.
- 약 45m 이동 또는 10초 경과 시 현재 GPS 주변 360m의 OSM 건물/식생을 비동기 프리패치하고 새 geometry가 오면 즉시 그림자를 갱신한다.
- viewport 건물 debounce를 140ms → 90ms로 줄여 지도 이동 후 그림자 반응을 조금 더 빠르게 했다.

## 회귀 보호
- v1.9.4.34 실시간 compass 화살표 유지
- 보행 중 경로 중간 maneuver marker 제거 유지
- 카메라 위/아래 튐 방지 유지
- Cloudflare Worker / Shade Graph / PWA / OSM + Overture + optional VWorld 유지
- route-first architecture 및 Destination Lock 유지

## 실기기 확인 포인트
1. 경로 없이 20~30m 이동 시 새로고침 없이 원형 현재 위치가 따라오는지
2. 기본 화면 속도가 0.0 → 실제 보행 km/h로 1~2개 GPS fix 안에 반응하는지
3. 7초 이상 watch가 멎는 환경에서 watchdog이 위치를 다시 갱신하는지
4. 이동 후 새 주변 건물/그림자가 이전보다 빠르게 나타나는지
5. 따라가기 시작 시 기존 실시간 compass 화살표가 정상 유지되는지

**End of ShadeWay Development Handoff — v1.9.4.35**
