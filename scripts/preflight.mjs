...
if(!html.includes('v1.9.4.35'))throw new Error('HTML version mismatch');
...
if(!html.includes('function startLiveLocationTracking'))throw new Error('Always-live GPS service missing');
if(!html.includes('function forceLiveLocationRefresh'))throw new Error('GPS stale-fix watchdog missing');
if(!html.includes('function scheduleLiveEnvironmentRefresh'))throw new Error('Live environment/shadow refresh missing');
if(!html.includes('id="liveSpeedValue"'))throw new Error('No-route live km/h UI missing');
if((html.match(/navigator\.geolocation\.watchPosition\(/g)||[]).length!==1)throw new Error('Expected one shared geolocation watch');
const sw=html.indexOf('function startWalk(){'),ew=html.indexOf('\nfunction stopWalk',sw),es=html.indexOf('\nasync function rerouteFromCurrent',ew);
if(html.slice(sw,ew).includes('watchPosition('))throw new Error('Route-only geolocation watch regression');
if(html.slice(ew,es).includes('clearWatch('))throw new Error('Stopping guidance must not stop global GPS watch');
...
if(!String(manifest.name||'').includes('1.9.4.35'))throw new Error('Manifest version mismatch');
